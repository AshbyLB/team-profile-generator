...
function addMembers() {
    inquirer.prompt([
        ...
        if (data.addTeamMember === 'Engineer') {
            ...
            fs.writeFile('../generated/test.html', renderPage(), (err) => err ? console.log(err) : console.log('test.html Created!'));
        }
    })
}
...
<body>
    ...
    for (let i = 0; i < teamMems.length; i++) {
        ...
        <div class="bg-success">
            <h5 class="card-title ml-2 mt-2">${teamMemData.getName()}</h5>
            <h6 class="card-subtitle ml-2 mb-2 "><i class="fas fa-user-graduate"></i>
                ${teamMemData.getRole()}</h6>
        </div>
        <div class="card-body">
            <ul class="list-group list-group-flush">
        ...
        <div class="bg-success">
            <h5 class="card-title ml-2 mt-2">${teamMemData.getName()}</h5>
            <h6 class="card-subtitle ml-2 mb-2 "><i class="fas fa-glasses"></i>
                ${teamMemData.getRole()}</h6>
        </div>
        <div class="card-body">
            <ul class="list-group list-group-flush">
                <li class="list-group-item">ID: ${teamMemData.getId()}</li>
                <li class="list-group-item"><a href="mailto:${teamMemData.getEmail()}">Send email</a></li>
                <li class="list-group-item">GitHub Name: <a href=https://github.com/${teamMemData.getGitHub()}>GitHub Profile</a></li>
            </ul>
        </div>
        </div>`
            renderPg.push(memStr);
